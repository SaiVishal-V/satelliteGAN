...
# DINCAE uses TensorFlow v1 graph/session APIs.
# Disable eager mode for compatibility when running on TensorFlow 2.x.
...
"""
    ...
    for i in range(ndata):
        ...
        if data_full[i].shape != data_full[0].shape:
            raise ValueError("shape are not coherent")
    ...
    def datagen():
        ...
        for i in range(ntime):
            ...
            if train:
                ...
                # add jitter
                for j in range(ndata):
                    jitter_scale = np.float32(jitter_std[j])
                    noise = rng.standard_normal((sz[1],sz[2]), dtype=np.float32)
                    noise *= jitter_scale
                    xin[:,:,2*j] += noise

                    noise = rng.standard_normal((sz[1],sz[2]), dtype=np.float32)
                    noise *= jitter_scale
                    xin[:,:,2*j + 2*ndata + 4] += noise

                    noise = rng.standard_normal((sz[1],sz[2]), dtype=np.float32)
                    noise *= jitter_scale
                    xin[:,:,2*j + 4*ndata + 4] += noise
...
def savesample(fname,m_rec,σ2_rec,meandata,lon,lat,e,ii,offset,
               transfun = (identity, identity), time = None):
    fill_value = -9999.
    ...
    if ii == 0:
        ...
        if time is not None:
            root_grp.createDimension('date_strlen', 10)
        ...
        if time is not None:
            nc_date_stamp = root_grp.createVariable(
                'date_stamp', 'S1', ('time', 'date_strlen'))
            nc_date_stamp.long_name = 'date stamp'
            nc_date_stamp.format = 'DD-MM-YYYY'
        ...
        nc_date_stamp = root_grp.variables.get('date_stamp')
...
#            recdata[n,:,:],meandata.mask)
        ...
        if time is not None and nc_date_stamp is not None:
            date_value = time[n + offset]
            if hasattr(date_value, "strftime"):
                date_string = date_value.strftime("%d-%m-%Y")
            else:
                date_string = str(date_value)[:10]
            nc_date_stamp[n+offset:n+offset+1,:] = np.asarray(
                [list(date_string)], dtype='S1')
...
def reconstruct(lon,lat,mask,meandata,
                ...
                save_best_output = False,
                best_output_filename = "best_checkpoint.nc",
 ...
 * `validate_each`: run validation every `validate_each` epochs. Set to zero to disable validation monitoring.
 * `early_stopping_patience`: stop training if validation RMS does not improve for this many validations. Set to zero to disable.
 * `early_stopping_min_delta`: minimum RMS improvement required to reset early-stopping patience.
 * `save_best_model`: save `best_checkpoint/best_checkpoint.ckpt` whenever validation RMS improves.
 * `save_best_output`: save a reconstruction NetCDF when validation RMS improves.
 * `best_output_filename`: output filename used when `save_best_output` is enabled.
 * `restore_latest_checkpoint`: resume from the latest checkpoint in `outdir` when available.
 * `restore_best_checkpoint`: resume from `best_checkpoint` instead of the latest
      periodic checkpoint when best-checkpoint epoch metadata show it is newer.
"""
    ...
    if iseed != None:
        ...
        tf.compat.v1.set_random_seed(np.random.randint(0, max_int32))
        random.seed(np.random.randint(0, max_int32))
    ...
    output_signature = (
        tf.TensorSpec(shape=(jmax, imax, nvar), dtype=tf.float32),
        tf.TensorSpec(shape=(jmax, imax, 2), dtype=tf.float32),
    )
    ...
    train_output_types = tf.compat.v1.data.get_output_types(train_dataset)
    ...
    test_iterator = tf.compat.v1.data.Iterator.from_structure(train_output_types,
                                                    train_output_shapes)
    ...
    iterator = tf.compat.v1.data.Iterator.from_string_handle(
            handle, train_output_types, output_shapes = train_output_shapes)
    ...
    for l in range(1,enc_nlayers):
        enc_conv[l] = tf.compat.v1.keras.layers.Conv2D(
            filters=enc_nfilter[l],
            kernel_size=(3,3),
            padding='same',
            activation=conv_activation)(enc_avgpool[l-1])
        print("encoder: output size of convolutional layer: ",l,enc_conv[l].shape)

        enc_avgpool[l] = tf.compat.v1.keras.layers.AveragePooling2D(
            pool_size=(2,2),
            strides=(2,2),
            padding='same')(enc_conv[l])
    ...
    if len(frac_dense_layer) == 0:
        ...
        for i in range(2*len(frac_dense_layer)):
            dense[2*i+1] = tf.compat.v1.keras.layers.Dense(
                units=dense_units[i],
                activation=tf.nn.relu)(dense[2*i])
            print("dense layer: output units: ",i,dense[2*i+1].shape)
            dense[2*i+2] = tf.nn.dropout(dense[2*i+1], rate=dropout_rate)
    ...
    for l in range(1,enc_nlayers):
        ...
        dec_conv[l] = tf.compat.v1.keras.layers.Conv2D(
            filters=dec_nfilter[l],
            kernel_size=(3,3),
            padding='same',
            activation=conv_activation)(dec_upsample[l])
    ...
    n_noncloud_safe = tf.maximum(n_noncloud, tf.constant(1.0, dtype=xtrue.dtype))
    has_noncloud = tf.greater(n_noncloud, 0)
    ...
    if truth_uncertain:
        ...
        cost = (tf.reduce_sum(input_tensor=tf.multiply(
            tf.math.log(σ2_rec/σ2_true) + (σ2_true + difference**2) / σ2_rec,mask_noncloud))) / n_noncloud_safe
    else:
        cost = (tf.reduce_sum(input_tensor=tf.multiply(tf.math.log(σ2_rec),mask_noncloud)) +
            tf.reduce_sum(input_tensor=tf.multiply(difference**2 / σ2_rec,mask_noncloud))) / n_noncloud_safe
    ...
    start_epoch = 0
    restored_best_epoch = None
    restored_best_val_rms = np.inf

    if restore_latest_checkpoint:
        def checkpoint_epoch_from_name(checkpoint_path):
        ...
        def read_best_checkpoint_metadata(best_dir):
            epoch_file = os.path.join(best_dir, "best_checkpoint_epoch.txt")
            if not os.path.exists(epoch_file):
                return None, None
            try:
                with open(epoch_file, "r", encoding="utf-8") as f:
                    epoch = int(f.readline().strip())
                    val_rms_line = f.readline().strip()
                    val_rms = float(val_rms_line) if val_rms_line else None
                    return epoch, val_rms
            except (OSError, ValueError):
                logger.warning("Could not read best checkpoint epoch metadata")
                return None, None

        checkpoint_candidates = []

        # Try to restore from the last saved periodic checkpoint.
        checkpoint_dir = outdir
        ckpt = tf.compat.v1.train.get_checkpoint_state(checkpoint_dir)

        if ckpt and ckpt.model_checkpoint_path:
            checkpoint_epoch = checkpoint_epoch_from_name(ckpt.model_checkpoint_path)
            if checkpoint_epoch is not None:
                checkpoint_candidates.append(
                    (checkpoint_epoch, ckpt.model_checkpoint_path, "latest checkpoint", None)
                )
            else:
                logger.warning("Could not extract epoch number from checkpoint filename")

        if restore_best_checkpoint and outdir is not None:
            best_dir = os.path.join(outdir, "best_checkpoint")
            best_ckpt = tf.compat.v1.train.get_checkpoint_state(best_dir)
            best_epoch, best_val_rms = read_best_checkpoint_metadata(best_dir)
            if best_ckpt and best_ckpt.model_checkpoint_path and best_epoch is not None:
                checkpoint_candidates.append(
                    (best_epoch, best_ckpt.model_checkpoint_path, "best checkpoint", best_val_rms)
                )

        if checkpoint_candidates:
            start_epoch, checkpoint_path, checkpoint_label, checkpoint_val_rms = max(
                checkpoint_candidates,
                key=lambda item: item[0],
            )
            logger.info(f"Restoring from {checkpoint_label}: {checkpoint_path}")
            saver.restore(sess, checkpoint_path)
            if checkpoint_label == "best checkpoint" and checkpoint_val_rms is not None:
                restored_best_epoch = start_epoch
                restored_best_val_rms = checkpoint_val_rms
        else:
            logger.info("No checkpoint found, starting training from scratch")
    else:
        logger.info("Checkpoint restore disabled; starting training from scratch")

    # final output file name
    fname = None

    best_val_rms = restored_best_val_rms
    ...
    if validate_each < 0:
        raise ValueError("validate_each must be >= 0")

    # loop over epochs
    for e in range(start_epoch, epochs):
        stop_training = False

        # collect per-epoch training losses to compute an epoch mean
        epoch_losses = []
        ...
        for ii in range(ceil(train_len / batch_size)):
            ...
            summary, batch_cost, batch_RMS, batch_n_noncloud, batch_learning_rate, _ = sess.run(
                [merged, cost, RMS, n_noncloud, learning_rate_, opt],feed_dict={
            ...
            if ii % 20 == 0:
                ...
                print("Epoch: {}/{}...".format(e+1, epochs),
                      ...
                      "RMS: {:.20f}".format(batch_RMS),
                      "observations: {:.0f}".format(batch_n_noncloud),
                      batch_learning_rate )

        if (validate_each > 0) and (((e + 1) % validate_each == 0) or (e == epochs - 1)):
            # Evaluate over the full test set to monitor generalization and stop
            # before late-epoch degradation.
            sess.run(test_iterator_init_op)
            val_rms_values = []
            val_cost_values = []
            val_n_noncloud_values = []

            for ii in range(ceil(test_len / batch_size)):
                val_cost, val_rms, val_n_noncloud = sess.run(
                    [cost, RMS, n_noncloud],
                    feed_dict={
                ...
                if val_n_noncloud > 0:
                    val_cost_values.append(val_cost)
                    val_rms_values.append(val_rms)
                    val_n_noncloud_values.append(val_n_noncloud)

            if val_n_noncloud_values:
                val_n_noncloud_values = np.asarray(val_n_noncloud_values, dtype=np.float64)
                val_rms_values = np.asarray(val_rms_values, dtype=np.float64)
                val_cost_values = np.asarray(val_cost_values, dtype=np.float64)
                epoch_val_rms = float(np.sqrt(np.sum((val_rms_values ** 2) * val_n_noncloud_values) / np.sum(val_n_noncloud_values)))
                epoch_val_cost = float(np.sum(val_cost_values * val_n_noncloud_values) / np.sum(val_n_noncloud_values))
            else:
                epoch_val_rms = np.inf
                epoch_val_cost = np.inf
            print(
                "Epoch: {}/{}...".format(e+1, epochs),
                "Validation cost: {:.20f}".format(epoch_val_cost),
                "Validation RMS: {:.20f}".format(epoch_val_rms),
            )

            # persist epoch-level metrics to CSV in outdir if provided
            if outdir is not None:
                try:
                    metrics_csv = os.path.join(outdir, "training_metrics.csv")
                    write_header = not os.path.exists(metrics_csv)
                    with open(metrics_csv, "a", newline="", encoding="utf-8") as mf:
                        writer = csv.writer(mf)
                        if write_header:
                            writer.writerow(["epoch", "train_loss_mean", "val_rms"])
                        epoch_train_loss = float(np.nan) if len(epoch_losses) == 0 else float(np.nanmean(np.asarray(epoch_losses, dtype=float)))
                        writer.writerow([e + 1, epoch_train_loss, epoch_val_rms])
                except Exception as ex:
                    logger.warning(f"Could not write training metrics CSV: {ex}")

            improved = epoch_val_rms < (best_val_rms - early_stopping_min_delta)
            if improved:
                best_val_rms = epoch_val_rms
                best_epoch = e + 1
                n_bad_epochs = 0

                if save_best_model and outdir != None:
                    best_dir = os.path.join(outdir, "best_checkpoint")
                    os.makedirs(best_dir, exist_ok=True)
                    best_path = best_saver.save(
                        sess,
                        os.path.join(best_dir, "best_checkpoint.ckpt"),
                    )
                    with open(os.path.join(best_dir, "best_checkpoint_epoch.txt"), "w", encoding="utf-8") as f:
                        f.write("{}\n{:.17g}\n".format(best_epoch, best_val_rms))
                    print("Saved best model at epoch {} with validation RMS {:.8f}: {}".format(
                        best_epoch,
                        best_val_rms,
                        best_path,
                    ))

                if save_best_output and outdir != None:
                    best_dir = os.path.join(outdir, "best_checkpoint")
                    os.makedirs(best_dir, exist_ok=True)
                    best_nc_path = os.path.join(best_dir, best_output_filename)
                    sess.run(test_iterator_init_op)
                    for ii in range(ceil(test_len / batch_size)):
                        batch_m_rec, batch_σ2_rec = sess.run(
                            [m_rec, σ2_rec],
                            feed_dict={
                                handle: test_iterator_handle,
                                mask_issea: mask,
                    ...
                    print("Saved best reconstruction at epoch {}: {}".format(best_epoch, best_nc_path))
            else:
                n_bad_epochs += 1

            if (early_stopping_patience > 0) and (n_bad_epochs >= early_stopping_patience):
                print(
                    "Early stopping triggered at epoch {}. Best validation RMS {:.8f} at epoch {}".format(
                        e + 1,
                        best_val_rms,
                        best_epoch,
                    )
        ...
        if (
            ...
            and (
                stop_training
                or (e == epochs-1)
                or ((save_each > 0) and ((e + 1) % save_each == 0))
            )
        ):
            epoch = e + 1
            print("Save output", epoch)

            fname = os.path.join(outdir,"data-epoch-{:03d}.nc".format(epoch))
            ...
            for ii in range(ceil(test_len / batch_size)):
                ...
                savesample(fname,batch_m_rec,batch_σ2_rec,meandata,lon,lat,e,ii,
                           offset, transfun = transfun, time = time)

        if (
            outdir != None
            and (
                stop_training
                or ((save_model_each > 0) and ((e + 1) % save_model_each == 0))
            )
        ):
    ...
    if best_epoch is not None:
        print("Best validation RMS {:.8f} at epoch {}".format(best_val_rms, best_epoch))
